...
func TestUploadBackoff(t *testing.T) {
	...
	terminateConnectionAt := atomic.NewUint64(700)

	faultyStreamer, err := events.NewCallbackStreamer(events.CallbackStreamerConfig{
		Inner: streamer,
		OnEmitAuditEvent: func(ctx context.Context, sid session.ID, event events.AuditEvent) error {
			terminateAt := int64(terminateConnectionAt.Load())
	...
	for i := 0; i < attempts; i++ {
		...
		// After in Scan has been called two times,
	...
	for i, diff := range diffs {
		if i > 0 {
			assert.Equal(t, true, diff > diffs[i-1], "Expected next retry to take longer, got %v vs %v", diffs[i-1], diff)
		}
	}
...
func TestUploadBadSession(t *testing.T) {
	...
	case event = <-eventsC:
		assert.Error(t, event.Error)
		assert.Equal(t, true, isSessionError(event.Error))
	case <-ctx.Done():
		t.Fatalf("Timeout waiting for async upload, try `go test -v` to get more logs for details")
	}
	...
	assert.NoError(t, err)
	// Bad record have been scanned, but upload have not started
